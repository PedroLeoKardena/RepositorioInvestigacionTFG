...
if __name__ == "__main__":
    ...
    print("=== EXTRACCIÓN DE MFCCs — AUDIOS CHUNKEADOS DIARIZADOS ===\n")

    print("--- Train ---")
    datos_train = procesar_dataset(ruta_entrenamiento / "metadata_train_chunked_diarizado.csv", ruta_audios)
    if datos_train:
        ruta_out = ruta_entrenamiento / "train_mfcc_chunked_diarizado.pkl"
        with open(ruta_out, 'wb') as f:
            pickle.dump(datos_train, f)
        print(f"Guardado: {ruta_out}")

    print("\n--- Test ---")
    datos_test = procesar_dataset(ruta_entrenamiento / "metadata_test_chunked_diarizado.csv", ruta_audios)
    if datos_test:
        ruta_out = ruta_entrenamiento / "test_mfcc_chunked_diarizado.pkl"
        with open(ruta_out, 'wb') as f:
