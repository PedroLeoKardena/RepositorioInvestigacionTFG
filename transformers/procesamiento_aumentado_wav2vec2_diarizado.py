...
def entrenar_modelo():
    ...
    ruta_audios = str(ruta_base / "audios_aumentados_diarizados")
    ruta_modelos = ruta_base / "modelos_entrenados"

    ruta_csv_train = ruta_entrenamiento / "metadata_train_aumentado_diarizado.csv"
    ruta_csv_test = ruta_entrenamiento / "metadata_test_aumentado_diarizado.csv"

    try:
        df_train = pd.read_csv(ruta_csv_train, sep=";")
        df_test = pd.read_csv(ruta_csv_test, sep=";")
    except FileNotFoundError:
        print("Archivos de metadata no encontrados. Asegúrate de haber ejecutado previamente generar_dataset_aumentado_diarizado.py")
        return
    ...
    train_dataset = train_dataset.map(
        ...
        batched=True, batch_size=8, remove_columns=['nombre_archivo']
    )
    ...
    for fold_val in range(5):
        ...
        training_args_cv = TrainingArguments(
            output_dir=str(ruta_modelos / f"wav2vec2_aumentado_diarizado_fold_{fold_val}"),
            eval_strategy="epoch", save_strategy="no",
            learning_rate=3e-5, per_device_train_batch_size=4,
            per_device_eval_batch_size=4, gradient_accumulation_steps=2,
            num_train_epochs=5, weight_decay=0.01, logging_steps=10,
            remove_unused_columns=False,
        )
        trainer_cv = MultiTaskTrainer(
            model=modelo_cv, args=training_args_cv,
            train_dataset=train_fold_ds, eval_dataset=val_fold_ds,
        )
        trainer_cv.train()
    ...
    print("Iniciando Entrenamiento Final con TODOS los datos de Train...")
    ...
    training_args_final = TrainingArguments(
        output_dir=str(ruta_modelos / "entrenamiento_final_wav2vec2_aumentado_diarizado"),
        eval_strategy="no", save_strategy="no",
        learning_rate=3e-5, per_device_train_batch_size=4,
        gradient_accumulation_steps=2, num_train_epochs=5,
        weight_decay=0.01, logging_steps=10, remove_unused_columns=False,
    )
    trainer_final = MultiTaskTrainer(
        model=modelo_final, args=training_args_final, train_dataset=train_final_ds,
    )
    trainer_final.train()
    ...
    reporte_caja_str = classification_report(real_caja_list, preds_caja_list, labels=etiquetas_caja, target_names=le_caja.classes_, zero_division=0)
    reporte_caja_dict = classification_report(real_caja_list, preds_caja_list, labels=etiquetas_caja, target_names=le_caja.classes_, zero_division=0, output_dict=True)
    print(reporte_caja_str)

    resultados = {
        "fecha": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        "modelo": nombre_modelo,
        "datos": "diarizados_aumentados",
        "hiperparametros": {
    ...
    ruta_log = ruta_log_dir / "registro_resultados_aumentados_diarizado.json"
    if ruta_log.exists():
    ...
    print(f"\nResultados guardados en: {ruta_log}")

    ruta_guardado_final = ruta_modelos / "modelo_multitask_augmented_wav2vec2_diarizado"
    ...
    print(f"\nProceso completado. Modelo guardado en '{ruta_guardado_final}'")
