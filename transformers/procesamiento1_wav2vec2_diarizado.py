...
def entrenar_modelo():
    ...
    ruta_audios = str(ruta_base / "audios_chunks_diarizados")
    ruta_modelos = ruta_base / "modelos_entrenados"

    ruta_csv_train = ruta_entrenamiento / "metadata_train_chunked_diarizado.csv"
    ruta_csv_test = ruta_entrenamiento / "metadata_test_chunked_diarizado.csv"

    try:
        df_train = pd.read_csv(ruta_csv_train, sep=";")
        df_test = pd.read_csv(ruta_csv_test, sep=";")
    except FileNotFoundError:
        print("Archivos de metadata no encontrados. Asegúrate de haber ejecutado previamente chunking_audios_diarizado.py")
        return
    ...
    for fold_val in range(5):
        ...
        training_args_cv = TrainingArguments(
            output_dir=str(ruta_modelos / f"wav2vec2_diarizado_fold_{fold_val}"),
            eval_strategy="epoch", save_strategy="no",
            learning_rate=5e-5, per_device_train_batch_size=4,
            per_device_eval_batch_size=4, gradient_accumulation_steps=2,
            num_train_epochs=10, weight_decay=0.01, logging_steps=10,
    ...
    training_args_final = TrainingArguments(
        output_dir=str(ruta_modelos / "entrenamiento_final_wav2vec2_diarizado"),
    ...
    ruta_log = ruta_log_dir / "registro_resultados_diarizado.json"
    if ruta_log.exists():
    ...
    ruta_guardado_final = ruta_modelos / "modelo_multitask_wav2vec2_diarizado"
